  ...
  function startSeller(initial){
    ...
    function pageMeta(){
      return{
        ...
        shops:['Mis comercios','Tu cartera, contactos, seguimientos y ventas en un solo lugar.'],
        routes:['Mi recorrido','Ordená visitas y llegá rápido a cada comercio.'],
    ...
    function renderHome(){
      ...
      const sales=validOrders.reduce((a,o)=>a+Number(o.total||0),0);
      const c=commission();
      const att=attention();
      let html='<section class="sp-welcome"><div><span class="sp-eyebrow light">MI JORNADA</span><h2>Hola, '+esc((state.data.viewer.name||'').split(' ')[0]||'vendedor')+'</h2><p>Tenés todo listo para salir, registrar visitas y cerrar pedidos.</p></div><div class="sp-welcome-actions"><button class="sp-btn white" data-action="new-shop">'+icon('store',17)+' Nuevo comercio</button><button class="sp-btn orange" data-action="new-order">'+icon('bag',17)+' Nuevo pedido</button></div></section>';
      html+='<div class="sp-kpis">'+kpi('Mis comercios',shops().length,shops().filter(s=>s.status==='Cliente').length+' clientes activos','store','blue')+kpi('Pedidos activos',active.length,active.reduce((a,o)=>a+Number(o.units||0),0)+' chapitas en marcha','bag','orange')+kpi('Vendido',money(sales),validOrders.length+' pedidos válidos','wallet','teal')+kpi('Mi comisión',money(c.due),c.units+' chapitas cobradas','user','purple')+'</div>';
      html+='<section class="sp-panel sp-attention"><div class="sp-panel-head"><div><span class="sp-tag">PRIORIDAD</span><h2>Qué atender ahora</h2><p>Seguimientos y oportunidades que conviene mover.</p></div><span class="sp-count">'+att.length+'</span></div>';
      if(att.length){
        html+='<div class="sp-attention-list">'+att.slice(0,5).map(s=>{
          const st=shopStats(s.id);const overdue=s.next_visit&&s.next_visit<=today();
          return '<div class="sp-attention-row"><i>'+icon('store',18)+'</i><div class="grow"><div class="sp-row-title"><button data-action="shop-detail" data-id="'+esc(s.id)+'">'+esc(s.name)+'</button>'+badge(s.status)+'</div><small>'+esc(s.zone||s.address||'Sin zona')+(s.next_visit?' · '+(overdue?'visita pendiente ':'próxima ')+date(s.next_visit):'')+(st.orders?' · '+st.orders+' pedidos':'')+'</small></div><div class="sp-row-actions">'+((s.map_url||s.address)?'<a class="sp-icon-btn" href="'+esc(directions(s))+'" target="_blank" rel="noreferrer">'+icon('nav',16)+'</a>':'')+'<button class="sp-icon-btn" data-action="new-visit" data-id="'+esc(s.id)+'">'+icon('clip',16)+'</button><button class="sp-btn tiny" data-action="new-order" data-id="'+esc(s.id)+'">Pedido</button></div></div>';
        }).join('')+'</div>';
    ...
    async function submitForm(form){
      ...
      try{
        if(state.modal==='shop'){
          ...
          await mutate('shop',d);toast('Comercio guardado');
          ...
          if(!d.shop_id)throw new Error('Elegí un comercio.');
          ...
          if(!shopId)throw new Error('Elegí un comercio.');
