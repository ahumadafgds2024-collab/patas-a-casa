(function sellerApp(){
  ...
  function startSeller(initial){
    ...
    function renderHome(){
      ...
      if(att.length){
        html+='<div class="sp-attention-list">'+att.slice(0,3).map(s=>{
          ...
          return '<div class="sp-attention-row"><i>'+icon('store',18)+'</i><div class="grow"><div class="sp-row-title"><button data-action="shop-detail" data-id="'+esc(s.id)+'">'+esc(s.name)+'</button>'+badge(s.status)+'</div><small>'+esc(s.zone||s.address||'Sin zona')+(s.next_visit?' · '+(overdue?'visita pendiente ':'próxima ')+date(s.next_visit):'')+'</small></div><div class="sp-row-actions">'+((s.map_url||s.address)?'<a class="sp-icon-btn" href="'+esc(directions(s))+'" target="_blank" rel="noreferrer" title="Cómo llegar">'+icon('nav',16)+'</a>':'')+'<button class="sp-btn secondary tiny" data-action="new-visit" data-id="'+esc(s.id)+'">Registrar visita</button><button class="sp-btn primary tiny" data-action="new-order" data-id="'+esc(s.id)+'">Pedido</button></div></div>';
        }).join('')+'</div>';
    ...
    function openShop(s){
      state.form=s?{...s}:{name:'',address:'',zone:'',contact:'',phone:'',status:'Pendiente',next_visit:'',map_url:'',notes:'',order_delivery:'',order_notes:''};
      state.items=[];state.orderModel=models[0];state.shopOrder=false;
    ...
    root.addEventListener('change',e=>{const field=e.target?.dataset?.field;if(field){state.form[field]=e.target.value;if(state.modal==='shop'&&field==='status')render()}});
    ...
    root.addEventListener('click',async e=>{
      ...
      else if(action==='shop-order-yes'){state.shopOrder=true;state.form.status='Cliente';render()}
      else if(action==='shop-order-no'){state.shopOrder=false;render()}
