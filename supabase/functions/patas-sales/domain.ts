...
export function commissionSummary(orders:any[],payments:any[],commissions:any[],sellerId:string){const units=orders.filter(o=>o.seller_id===sellerId&&!['Recibido','Cancelado'].includes(o.status)).reduce((a,o)=>a+o.units,0);const earned=units*1750;const paid=commissions.filter(c=>c.seller_id===sellerId).reduce((a,c)=>a+c.amount,0);return{units,earned,paid,due:Math.max(0,earned-paid),remainder:units%4};}
