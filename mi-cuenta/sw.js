...
self.addEventListener("fetch",event=>{
  ...
  if(url.origin!==self.location.origin || !url.pathname.startsWith("/mi-cuenta/")) return;
  event.respondWith(fetch(req).then(res=>{
    const copy=res.clone();
    if(res.ok) caches.open(CACHE).then(c=>c.put(req,copy));
    return res;
  }).catch(()=>caches.match(req).then(r=>r||caches.match("/mi-cuenta/"))));
});
