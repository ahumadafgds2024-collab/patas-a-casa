(function sellerApp(){
  ...
  function startSeller(initial){
    ...
    function renderHome(){
      ...
      html+='</section>';
      html+='<section class="sp-quick-panel"><div class="sp-panel-head"><div><span class="sp-tag">ACCESOS RÁPIDOS</span><h2>Resolver en pocos toques</h2><p>Solo las acciones que más usa un vendedor en la calle.</p></div></div><div class="sp-quick-grid">'+quick('new-shop','Nuevo local','Guardá el local donde estás.','store',false)+quick('new-visit','Registrar visita','Qué pasó y cuándo volver.','clip',!shops().length)+quick('new-order','Nuevo pedido','Tipo, color y cantidad.','bag',!shops().length)+'</div></section>';
      html+='<div class="sp-kpis">'+kpi('Mis locales',shops().length,shops().filter(s=>s.status==='Cliente').length+' clientes activos','store','blue')+kpi('Pedidos activos',active.length,active.reduce((a,o)=>a+Number(o.units||0),0)+' chapitas en marcha','bag','orange')+kpi('Vendido',money(sales),validOrders.length+' pedidos válidos','wallet','teal')+kpi('Mi comisión',money(c.due),c.units+' chapitas cobradas','user','purple','money')+'</div>';
      const recent=matchingShops().slice(0,4);
      html+='<section class="sp-section"><div class="sp-section-head"><div><h2>Mis locales</h2><p>Acceso rápido a tu cartera.</p></div><button class="sp-link" data-nav="shops">Ver todos '+icon('chevron',15)+'</button></div>'+renderShopCards(recent,true)+'</section>';
      return html;
    }
