...
export function change(state:Row,user:Row,action:string,input:unknown,now=new Date().toISOString()){
 ...
 if(action==='shop'){const s=shopSchema.parse(input);if(admin){validSeller(s.seller_id)}else{if(s.id){const current=existing('shops',s.id);if(!current||current.seller_id!==seller.id)fail('Comercio no asignado a tu cuenta.',403)}s.seller_id=seller.id}const id=s.id||'PAC-'+crypto.randomUUID().slice(0,8).toUpperCase();upsert('shops',{...s,id,created:existing('shops',id)?.created||now});return {id};}
 if(action==='quickShop'){const x=z.object({name:z.string().trim().min(2).max(300),address:z.string().trim().max(300).default(''),zone:z.string().trim().max(300).default(''),map_url:z.string().trim().max(1000).default('')}).parse(input);const id='PAC-'+crypto.randomUUID().slice(0,8).toUpperCase();state.shops.push({id,name:x.name,address:x.address,zone:x.zone,contact:'',phone:'',seller_id:admin?'':seller.id,status:'Pendiente',next_visit:'',map_url:x.map_url,notes:'Creado al cargar un pedido',created:now});return{id};}
